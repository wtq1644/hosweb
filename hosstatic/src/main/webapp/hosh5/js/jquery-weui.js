...
+ function($) {
  ...
  $.prompt = function(text, title, onOK, onCancel, input, placeholder, maxlength , clazz , stylezz) {
    var config;
    if (typeof text === 'object') {
      config = text;
    } else {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><input type="text" class="weui-input weui-prompt-input" style="' + (config.stylezz || '') + '" id="weui-prompt-input" value="' + (config.input || '') + '"  placeholder="' + (config.placeholder || '') + '"   maxlength="' + (config.maxlength || '') + '"/>',
      title: config.title,
      autoClose: false,
      buttons: [
      {
...
+ function($) {
  ...
  defaults = $.fn.datetimePicker.prototype.defaults = {
    ...
    parse: function (str) {
      ...
      if(t[0].length == 4 && t.length == 3){//TODO 说明这是年月日，任建君
    	  return t[0].split(/\D/).concat(t[1].split(/:|时|分|秒/)).concat(t[2].split(/:|时|分|秒/)).filter(function (d) {
    		  return !!d;
    	  })
      }else{
